...
Page({
		  ...
		  header: {
		   ...
		   success:(res)=>{
				if(res.data.code == 200 && res.data.data.length > 0){
					...
					let url =  this.data.url + `/order/userPayOrder?patId=${res.data.data[0].id}`;
					if(this.data.type == 'index') url =  this.data.url + `/order/userPayOrder?patId=${res.data.data[0].id}&state=0`
					wx.request({
						url: url,
						header: {
						  'Authorization': token
	...
	changeValue(e) {
		...
		if(this.data.type == 'index') url =  this.data.url + `/order/userPayOrder?patId=${this.data.patList[e.detail.value].id}&state=0`
		wx.request({
			url: url,
			header: {
			  'Authorization': token
